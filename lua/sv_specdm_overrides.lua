...
hook.Add("TTTBeginRound", "TTTBeginRound_Ghost", function()
    ...
    function GAMEMODE:PlayerDeath(ply, infl, attacker)
        if ply:IsGhost() then
            HasteMode = function() return false end
...
hook.Add("Initialize", "Initialize_SpecDM", function()
    ...
    hook.Add("PlayerCanPickupWeapon", "TTTSpecDMPickup", function(ply, wep)
        if ply:IsGhost() then return string.Left(wep:GetClass(), #"weapon_ghost") == "weapon_ghost" end
    end)
    ...
    hook.Add("EntityTakeDamage", "EntityTakeDamage_Ghost", function(ent, dmginfo)
        if ent:IsPlayer() then
            ...
            if ent:IsGhost() and IsValid(dmginfo:GetInflictor()) and dmginfo:GetInflictor():GetClass() == "trigger_hurt" then return true end
        end
    end)
    ...
    if IsValid(ply) and ply:IsPlayer() and ply:IsGhost() then
        ...
        if math.floor(damage) > 0 then
            ...
            if damage > 5 then
                ...
                -- bots don't need to hear the sound
...
hook.Add("AcceptInput", "AcceptInput_Ghost", function(ent, name, activator, caller, data)
    if IsValid(caller) and caller:GetClass() == "ttt_logic_role" and IsValid(activator) and activator:IsPlayer() and activator:IsGhost() then return true end
end)

hook.Add("EntityEmitSound", "EntityEmitSound_SpecDM", function(t)
    if t.Entity and t.Entity:IsPlayer() and t.Entity:IsGhost() and t.OriginalSoundName == "HL2Player.BurnPain" then return false end
end)

hook.Add("EntityTakeDamage", "GhostDamages_SpecDM", function(ent, dmginfo)
    local atk = dmginfo:GetAttacker()
    if (IsValid(ent) and IsValid(atk) and atk:IsPlayer() and atk:IsGhost()) and (not ent:IsPlayer() or not ent:IsGhost()) then return true end
end)
