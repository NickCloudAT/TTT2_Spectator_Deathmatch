...
hook.Add("ShouldCollide", "ShouldCollide_Ghost", function(ent1, ent2)
    if IsValid(ent1) and IsValid(ent2) then
        if ent1:IsPlayer() and ent1:IsGhost() and not (ent2:IsPlayer() and ent2:IsGhost()) then return false end
        if ent2:IsPlayer() and ent2:IsGhost() and not (ent1:IsPlayer() and ent1:IsGhost()) then return false end
    end
end)
...
hook.Add("Move", "Move_Ghost", function(ply, mv)
    if ply:IsGhost() then
        ...
        -- fix 1 for speedmodifier (shinigami noLag fix)
        local noLag = {1}
