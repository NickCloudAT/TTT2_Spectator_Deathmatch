...
net.Receive("SpecDM_QuakeSound", function()
    ...
    if timer.Exists("SpecDM_1") then
        timer.Remove("SpecDM_1")
    end

    if timer.Exists("SpecDM_2") then
        timer.Remove("SpecDM_2")
    end

    if timer.Exists("SpecDM_3") then
        timer.Remove("SpecDM_3")
    end
    ...
    if kills >= 3 then
        ...
        local x1 = surface.GetTextSize(text_1)
        ...
        local x2 = surface.GetTextSize(tbl[2])
        ...
        local x4 = surface.GetTextSize(text_4)
        ...
        if combos >= 2 and combos <= 5 then
            local text_12 = tbl_combos[combos][2]
            label_5 = vgui.Create("DLabel")
            label_5:SetFont("SpecDM_Quake")
            label_5:SetText(text_12)
            label_5:SizeToContents()
            label_5:SetTextColor(color_white)
            local x12 = surface.GetTextSize(text_12)
